...
\begin{theorem}
$\h(K,\lambda)$ is canonically an $\hecke_\finite$-module.
\end{theorem}
\begin{proof}
It is sufficient to construct an action of the double coset $g K h$ for 
$K\subset G(\dA_\finite)$ sufficiently small. 
\end{proof}

In our setting, it is the 
complex points of a variety (also denoted $\shimura_K(G)$) defined over $\dQ$. 
\[
  \operatorname{H}_l^\bullet(K,\lambda)
\]
