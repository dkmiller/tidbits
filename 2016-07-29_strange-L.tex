...
\usepackage{amsmath,amssymb,amsthm,bookmark,fixltx2e,thmtools}
...
\begin{definition}
Let $x\in \bR^r$. Following \cite{bugeaud-2008}, we say that the 
\emph{irrationality exponent} of $x$ is the supremum of all real numbers $\mu$ 
such that 
\end{definition}

\begin{theorem}
For $x\in \bR^d$, write $d(x,\bZ)=\min\{|x-n| : n\in \bZ\}$. For $x\in \bR$, we 
have the error bound:
\[
	\left|\sum_{n\leqslant N} e^{2\pi i \langle x,n m\rangle}\right| \leqslant\frac{1}{d(\langle x,m\rangle,\bZ)} .
\]
\end{theorem}
\begin{proof}

Note that 
$d(x,\bZ) \leqslant |\sin(\pi x)|$. This, combined with the duplication formula 
$\cos(2x)=1-2\sin^2(x)$ gives us
\[
	\left|\sum_{n\leqslant N} e^{2\pi i \langle x,n m\rangle}\right| \leqslant \frac{2}{|e^{2\pi i \langle x,m\rangle}-1|} = \frac{1}{|\sin(\pi \langle x,m\rangle)|} \leqslant \frac{1}{d(\langle x,m\rangle,\bZ)} .
\]
\end{proof}
