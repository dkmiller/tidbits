...
\begin{align*}
	...
	\frac{1}{d(m\cdot x,\bZ)} &\ll_{\epsilon,x} |m|^{\omega_{r-1}(x)+\epsilon}\qquad \text{as $m\to \infty$ in $\bZ^r$ .}
\end{align*}
\end{lemma}
\begin{proof}
...
\]
which implies $\frac{1}{d(m\cdot x,\bZ)} \ll |m|^{\omega_{r-1}(x)+\epsilon}$, 
...
\[
	\widehat f(m) = \int_{\bT^r} e^{2\pi i (m\cdot x)}\, \dd x .
\]


\begin{theorem}[Jarník]
...
\[
	\left| \sum_{n\leqslant N} e^{2\pi i n (m\cdot x)}\right| \ll_{\epsilon,x} |m|^{\omega_{r-1}(x)+\epsilon} 
\]
...
\[
	\left| \sum_{n\leqslant N} e^{2\pi in(m\cdot x)}\right|
		= \left|\frac{e^{2\pi i N (m\cdot x)}-1}{e^{2\pi i m\cdot x}-1}\right| 
		\leqslant \frac{2}{|e^{2\pi i m x}-1|} .
\]
Since $|e^{2\pi i m x}-1| = \sqrt{2-2\cos(2\pi m\cdot x)}$ and 
$\cos(2\theta)=1-2\sin^2(\theta)$, we obtain 
$\left|\sum_{n\leqslant N} e^{2\pi i n (m\cdot x)}\right| \leqslant \frac{1}{|\sin(\pi m\cdot x)|}$. 
It is easy to check that $|\sin(\pi t)|\geqslant d(t,\bZ)$, hence 
$\left|\sum_{n\leqslant N} e^{2\pi i n (m\cdot x)}\right| \leqslant \frac{1}{d(m\cdot x,\bZ)}$. 
...
\begin{theorem}
If $\omega_0(x)<\infty$, then the sequence $x_n=n x$ has discrepancy 
\[
	\disc(\bx^C) = \Omega(C^?) .
\]
\end{theorem}
