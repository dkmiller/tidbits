...
\title
  [Representations and deformation theory in arithmetic]
...
and the $p$-ordinary deformation space of a two-dimensional modular Galois 
representation. Hopefully this analogy should generalize to arbitrary 
reductive groups, or at least $\GL(n)$. In light of this, much of this note is 
an introduction to the relevant objects from a general perspective. So we 
introduce notion of a $p$-adic family of automorphic representations, and the 
(conjecturally) corresponding family of $p$-adic Galois representations in 
full generality. 
\end{abstract}
...
Recall that a morphism $f:X\to Y$ of 
...
when $x$ is clear. To save space, we will write $\pi_1(A)$ instead of 
$\pi_1(\spectrum A)$. A good reference for all of this is \cite{milneLEC}.
...
\cite[2.4]{mazur-1973} similar to 
the classical Artin-Verdier duality 
$\extension^\bullet(\sL,\dQ) = \h_c^{3-\bullet}(M,\sL)^\vee$ for local systems 
on a $3$-manifold $M$. 
...
the \emph{knot group} is $\Gamma_K=\pi_1(S^3\smallsetminus K)$, corresponding to 
$\Gamma_{\dQ,p} = \pi_1(\dZ[\frac 1 p])$. The peripheral map 
$\pi_1(V_K\smallsetminus K) \to \Gamma_K$ corresponds to the map 
$\pi_1(\dQ_p) \to \Gamma_{\dQ,p}$. 
...
\]
We can think of the degenerate case ``$p=1$'' as being in exact analogy with 
...
In \cite{c12}, it is shown that there is a unique fiber-product-preserving 
functor $(-)(\dA)$ from affine schemes of finite type over $F$ to topological spaces, 
...
Let $G$ be a connected reductive group over $F$, and let $\fg=\lie(G)$. There exists 
...
vectors which are also $Z(\fg_\infty)$-finite. (It is a non-trivial fact that 
$K$-finite vectors are smooth.)
...
\begin{equation}\label{eq:aut-decomp}
  \cA(G,\omega) = \bigoplus_{\pi\in \widehat G_\mathrm{c.a}(\omega)} m(\pi) \pi .
\end{equation}
...
\[
  \shimura_K(G) 
    \twoheadleftarrow \shimura_{K\cap C}(G) \xrightarrow{\cdot g} 
    \shimura_{K\cap g^{-1} C g}(G) \twoheadrightarrow
    \shimura_K(G) .
\]
...
Better:
\[
  \h^\bullet_\cusp(\shimura(G),\sV_\rho) = \bigoplus_{\pi\in \widehat G_\mathrm{c}(\omega)}\h^\bullet(\hecke_\infty,\pi_\infty\otimes\rho)\otimes \pi_\finite .
\]
...
$\Gamma_{\dQ,l n}=\pi_1(\dZ[\frac{1}{l n}])$ and 
$\hecke_\finite$. So $\Gamma_{\dQ,l n}$ acts on each $\hecke_\finite$-irreducible 
piece. These pieces are 2-dimensional, so we get, for each cuspidal eigenform 
$f$, a Galois representation 
$\rho_{f,l}:\Gamma_{\dQ,nl} \to \GL_2(\overline{\dQ_l})$. 
...
It is known that $\rho_{f,l}:\Gamma_{\dQ,l n} \to \GL_2(\overline{\dQ_l})$ 
...
\[
  \bar\rho_{f,l}:\Gamma_{\dQ,l n} \to \GL_2(O_{f,\lambda}/\lambda) = \GL_2(\dF_\lambda) .
\]
We say that a mod-$l$ representation of $\Gamma_\dQ$ is \emph{modular} (better, 
\emph{automorphic}) if it is of the form $\bar\rho_{f,l}$ for some $f$. 
Similarly, we say that an $l$-adic representation of $\Gamma_\dQ$ is 
...
$\rho_{\pi,p}:\Gamma_\dQ\to \langlands G(\overline{\dQ_p})$ unramified almost 
...
$\Gamma_\dQ\to \GL_n(\overline{\dQ_p})$. Note that even if multiplicity-one theorems 
...
$\rho_\dsigma:\Gamma_\dQ\to \langlands G(\sO(\fT))$ such for all 
$\sigma\in \dsigma^\classical$, the composite 
\[
  \rho_\sigma:\Gamma_\dQ\to \langlands G(\sO(\fT)) \xrightarrow{\mathrm{ev}_\sigma} \langlands G(\overline{\dQ_p}) 
\]
...
action of $\Gamma_\dQ$. The ``right'' way to see this is as follows. Spread out 
...
systems correspond to representations of $\pi_1(U)=\Gamma_{\dQ,S}$. 
...
representation $\rho:\Gamma_{\dQ,S}\to \GL_n(\dQ_l)$ that is suitably nice 
...
$\bar\rho:\Gamma_{\dQ,S} \to \GL_n(\dF_l)$ is automorphic. A theorem to the 
...
because $\Gamma_S=\pi_1(\spectrum(\dZ)\smallsetminus S)$ is not a finitely presented 
...
representations $\Gamma_S\to \GL_2(\dC)$, we should look for continuous 
representations $\Gamma_S\to \GL_2(A)$, where $A$ is a topological $\dZ_p$-algebra. 
...
character}. This is defined, for $\sigma\in \Gamma_\dQ$, by 
\[
...
\]
for some character $\varepsilon:\Gamma_K\to \dC^\times$. In particular, 
...
$\rho_{\bm f}:\Gamma_\dQ\to \GL_2(\hida^\circ)$, such that \emph{all} $p$-ordinary 
modular Galois representations congruent to $\rho_{f,p}$ come from 
$\hida^\circ$. 
...
of modular forms, and $\rho_{\bm f}:\Gamma_\dQ\to \GL_2(\hida^\circ)$ the Galois 
...
$f$ satisfies some technical hypotheses. This is the analogy of the map 
$T_K^\circ \to X_K^\circ$ being an isomorphism in a small neighborhood. 
